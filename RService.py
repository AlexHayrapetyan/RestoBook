...
@rest.route('/enteringEmail', methods=['GET', 'POST'])
def enteringEmail():
    if request.method == 'POST':
        email = request.form.get('email')
        ...
        if user:
            # Send verification digits to the user's email
            send_digits_email(email)
            # Store user_id in session for use in the newPassword route
            session['user_id'] = user.id
            return redirect(url_for('newPassword'))
        else:
            flash("Email not found. Please try again.")
            return render_template('enteringEmail.html')

    return render_template('enteringEmail.html')


# Route to reset password
@rest.route('/newPassword', methods=['GET', 'POST'])
def newPassword():
    if request.method == 'POST':
        try:
            ...
            truedigits = session.get('truedigits')  # Retrieve the correct digits from session
            ...
            # Validate digits
            if truedigits != digits:
                flash("Invalid digits input")
                return render_template('newPassword.html')
...
# Function to send verification digits to user's email
def send_digits_email(email):
    user = db.session.query(User).filter_by(email=email).first()
    if user:
        # Generate a random 4-digit code
        random_number = str(random.randint(1000, 9999))
        session['truedigits'] = random_number  # Store the digits in session for verification

        try:
            # Render email body with the 4-digit code
            body = render_template('digits.html', digits=random_number)
            ...
            mail.send(msg)
            flash("Verification digits have been sent to your email.")
        except Exception as e:
            logging.error(f"{e} occurred")
            flash("Failed to send the email. Please try again.")
    else:
        flash("User not found")
